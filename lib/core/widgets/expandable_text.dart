...
class ExpandableTextWidget extends StatefulWidget {
  const ExpandableTextWidget(
    ...
    this.selectionColor,
    this.trimTextInside = false,
    this.delimeterTappable = false,
    this.trimAlignment = Alignment.topLeft,
  })  : richData = null,
        richPostData = null;

  const ExpandableTextWidget.rich(
  ...
  /// Used trim inside text or not
  final bool trimTextInside;

  /// Used delimeter can collapse
  final bool delimeterTappable;

  final Alignment trimAlignment;
...
class ExpandableTextWidgetState extends State<ExpandableTextWidget> {
  ...
  Widget _builder(BuildContext context, bool isCollapsed, Widget? child) {
    ...
    final delimiter = TextSpan(
      ...
      recognizer: widget.delimeterTappable ? _recognizer : null,
    );
    ...
    Widget result = LayoutBuilder(
      builder: (BuildContext context, BoxConstraints constraints) {
        ...
        final textPainter = TextPainter(
          text: widget.trimTextInside ? link : const TextSpan(),
          textAlign: textAlign,
          ...
          case TrimMode.line:
            if (textPainter.didExceedMaxLines) {
              ...
              textSpan = TextSpan(
                children: <TextSpan>[
                  effectiveDataTextSpan,
                  if (widget.trimTextInside && linkLongerThanLine)
                    const TextSpan(text: _kLineSeparator),
                  delimiter,
                  if (widget.trimTextInside) link,
                ],
              );
        ...
        return Column(
          children: [
            Row(
              children: [
                Expanded(
                  child: textSpanResult,
                ),
              ],
            ),
            if (!widget.trimTextInside)
              Align(
                alignment: widget.trimAlignment,
                child: GestureDetector(
                  onTap: _recognizer.onTap,
                  child: Text(
